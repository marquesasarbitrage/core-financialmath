...
void writeNelsonSiegelResult(const std::shared_ptr<NelsonSiegelFamily>& ns, std::map<double, double> initialData, std::string fileName) {
    ...
    std::filesystem::create_directory("NelsonSigelResult");
    std::ofstream file("NelsonSigelResult/" + fileName +".csv");
    if (!file.is_open()) {
...
void testCanadianZeroYieldFit()
{
    ...
    std::cout << "Successful - find csv under build/NelsonSigelResult (canadaZeroCurveNelsonSiegel.csv and canadaZeroCurveSvensson.csv)." << std::endl;
}
